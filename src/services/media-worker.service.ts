...
export class MediaWorkerService extends BaseRabbitService {
  private static VIDEO_QUEUE_NAME =
    env.VIDEO_QUEUE_NAME || 'video_processing_queue';
  private static PHOTO_QUEUE_NAME =
    env.PHOTO_QUEUE_NAME || 'photo_processing_queue';
  ...
  public publishVideoProcessingRequest(
    videoId: number,
    videoPath: string,
    addresIP: string,
  ) {
  ...
  public publishPhotoProcessingRequest(
    photoId: number,
    photoPath: string,
    addresIP: string,
  ) {
