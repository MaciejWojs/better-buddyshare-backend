...
export class UserCacheDao {
    ...
    async findById(id: number): Promise<User | null> {
        ...
        if (cachedUser) {
            ...
            if (cachedUser.created_at) cachedUser.created_at = new Date(cachedUser.created_at);
            if (cachedUser.ban_expires_at) cachedUser.ban_expires_at = new Date(cachedUser.ban_expires_at);
            return cachedUser;
        }
    ...
    async upsertUser(user: User): Promise<User> {
        ...
        if (userFound) {
            if (isEqual(user, userFound)) {
                console.log(`User with ID ${id} has no changes. Skipping cache update.`);
                return userFound;
            }
            ...
            console.log(`User with ID ${id} already exists in cache. Updating cache with new data.`);
        }
