...
if (!env.MINIO_ENDPOINT || !env.MINIO_ROOT_USER || !env.MINIO_ROOT_PASSWORD) {
  console.error(
    'MinIO configuration is not fully defined in environment variables',
  );
  process.exit(1);
}
...
s3Client
  ...
  .then(() => {
    console.log(
      `Successfully connected to MinIO bucket "${bucket}" and wrote test file`,
    );
  })
  .catch((error: Error) => {
    console.error(
      `Error connecting to MinIO bucket "${bucket}" or writing test file:`,
      error,
    );
    process.exit(1);
  });
...
videoS3Client
  ...
  .then(() => {
    console.log(
      `Successfully connected to MinIO bucketVideo "${bucketVideo}" and wrote test file`,
    );
  })
  .catch((error: Error) => {
    console.error(
      `Error connecting to MinIO bucketVideo "${bucketVideo}" or writing test file:`,
      error,
    );
    process.exit(1);
  });
