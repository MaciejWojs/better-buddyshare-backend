...
-- =============================================================================
-- ! Important: validate data before insert (for all OBJECTS)

CREATE OR REPLACE FUNCTION bulk_insert_statistics_transaction(p_statistics JSONB[]) RETURNS INTEGER LANGUAGE plpgsql AS $$
DECLARE
    stat JSONB;
    inserted_count INTEGER := 0;
    new_stat stream_statistics_in_time%ROWTYPE;
BEGIN
    IF p_statistics IS NULL OR array_length(p_statistics, 1) = 0 THEN
        RAISE EXCEPTION 'Input statistics array cannot be null or empty.'
            USING ERRCODE = 'invalid_parameter_value';
    END IF;

    BEGIN
        FOREACH stat IN ARRAY p_statistics LOOP
            new_stat := add_stream_statistic(
                (stat->>'stream_id')::INT,
                (stat->>'stream_statistic_type_id')::INT,
                COALESCE((stat->>'value')::INT, 0),
                COALESCE((stat->>'timepoint')::TIMESTAMP(3), NOW())
            );
...
COMMENT ON FUNCTION bulk_insert_statistics_transaction(JSONB[]) IS 'Bulk inserts multiple statistic entries in a transaction. Rolls back all inserts if any exception occurs.';

CREATE OR REPLACE FUNCTION bulk_update_statistics_transactional(p_statistics JSONB[]) RETURNS INTEGER LANGUAGE plpgsql AS $$
DECLARE
    stat JSONB;
...
BEGIN
    ...
    BEGIN
        FOREACH stat IN ARRAY p_statistics LOOP
            stat_id := (stat->>'statistic_in_time_id')::INT;
            new_value := (stat->>'value')::INT;

            updated_row := update_stream_statistic_value(
                stat_id,
                new_value
            );
...
COMMENT ON FUNCTION bulk_update_statistics_transactional(JSONB[]) IS 'Bulk updates statistic entries in a single transaction using JSONB array with final values. Rolls back all updates if any record fails. Returns number of successfully updated records.';
